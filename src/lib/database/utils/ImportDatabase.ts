...
import { SmiteItemsApi } from "@lib/api/hirez/smite/SmiteItemsApi";
...
export async function importGods() {
    ...
    for (let god of data) {
        ...
        await container.prisma.gods.upsert({
            create: { ...godData, ...{ id: god.id } },
            update: godData,
            where: {
                id: god.id
            }
...
export async function importItems() {
    ...
    for (let item of data) {
        ...
        container.prisma.items.upsert({
            create: { ...itemData, ...{ id: item.ItemId } },
            update: itemData,
            where: {
                id: item.ItemId
            }
        })
            .then(item => container.logger.info(`Item ${item.name}<${item.id}> has been updated / inserted.`))
